...
class TSLARealtimeTracker:
    ...
    def _setup_stream_handlers(self):
        """Set up WebSocket stream handlers"""
        @self.stream_client.on("quotes")
        async def on_quote(data):
            """Handle incoming quote data"""
            self._process_quote_data(data)
        
        @self.stream_client.on("trades")
    ...
    def _process_trade_data(self, data):
        """Process streaming trade data"""
        trade_data = {
            'price': float(data.price),
            'size': data.size,
            'timestamp': data.timestamp
        }
        # Get latest quote to display with trade
    ...
    async def run_streaming(self):
        ...
        try:
            # Subscribe to TSLA quotes and trades
            self.stream_client.subscribe_quotes(self.symbol)
            self.stream_client.subscribe_trades(self.symbol)
